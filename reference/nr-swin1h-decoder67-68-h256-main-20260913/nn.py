"""Ordinary1h source-equation NN: full64-query windows, original sourcezero ingress.
...
def geometry(h,w,x,y):
 if any(type(i)is not int for i in (h,w,x,y)) or not(8<=h<=256 and 8<=w<=256 and h%8==w%8==0 and x in (-4,0) and y in (-4,0)):raise ValueError('admitted positive multiples8 and XY0/-4')
...
def tile_sources(h,w,x,y,cx,cy):
 ...
 off=np.arange(2048);tile=off//512;ty=(y+8*cy)//4+tile//2;tx=(x+8*cx)//4+tile%2;valid=(ty>=0)&(ty<h//4)&(tx>=0)&(tx<w//4)
 return np.where(valid,512*(ty*(w//4)+tx)+off%512,-1)

def ingress(features,*,h,w,x,y,cta):
 if type(features)is not bytes or len(features)!=h*w*32:raise ValueError('canonical-C32 packed feature span')
 mp=tile_sources(h,w,x,y,*cta);out=np.zeros(2048,dtype=np.uint8);valid=mp>=0;out[valid]=np.frombuffer(features,dtype=np.uint8)[mp[valid]];return out

def forward_window(features,slab,*,rsqrt,reciprocal):
 decoded=ffn.num.decode_e4m3(features,ffn.C_MAP).astype(np.float16);f=ffn.forward(decoded,slab);r=qkv.forward_raw(f['physical'].tobytes(),slab);n=normalized.forward(r['warp_bank_view'],slab,rsqrt=rsqrt);a=attention.forward(n['Q'],n['K'],n['V'],f['prequantization'],slab,reciprocal=reciprocal)
 return dict(ffn=f,raw=r,norm=n,attention=a,physical=a['physical'])

def forward(features,slab,*,h=64,w=64,x,y,rsqrt,reciprocal,keep_stages=False):
