"""CPU/static complete 128x4096 reference for ``cc_vit_1d_ffn_expand_fp8``.
...
def forward(input_fp8: bytes, weight_slab: bytes,
    ...
    and ``output_physical`` (the source's 524288-byte packed layout). Hooks
    ...
    if type(input_fp8) is not bytes or len(input_fp8) != INPUT_BYTES:
        raise ValueError('131072-byte packed A required')
