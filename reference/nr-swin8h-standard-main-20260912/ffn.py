...
def forward(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=16384 or type(slab)is not bytes or len(slab)!=689232:raise ValueError('own64x256/slab byte contract')
 ...
 gamma=np.frombuffer(slab,dtype='<f2',count=256,offset=360464).copy()
