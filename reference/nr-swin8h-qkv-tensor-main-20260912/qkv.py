...
A_MAP=axes.a_offset(np.arange(64)[:,None],np.arange(256)[None,:],256)
B_MAP=492544+axes.b_offset(np.arange(256)[:,None],np.arange(768)[None,:],768)
def forward_raw(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=16384 or type(slab)is not bytes or len(slab)!=820784:raise ValueError('own64x256/slab contract')
 a=num.decode_e4m3(features,A_MAP);b=num.decode_e4m3(slab,B_MAP);c=np.zeros((64,768),dtype=np.float16);before={};after={}
 ...
 banks=c.reshape(64,8,3,32);num._call_hook(hooks,'raw_projected_half',c);num._call_hook(hooks,'warp_bank_view',banks)
