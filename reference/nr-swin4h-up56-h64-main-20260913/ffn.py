...
EXP_MAP=16384*(h//128)+axes.b_offset(k.T,h%128,128)
GROUP_MAP=65536+4096*np.arange(4)[:,None,None]+axes.b_offset(np.arange(128)[None,:,None],np.arange(32)[None,None,:],32)
MIX_MAP=81920+axes.b_offset(k.T,k,C)
...
def forward(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=8192 or type(slab)is not bytes or len(slab)!=230176:raise ValueError('own64x256/slab byte contract')
 ...
 gamma=np.frombuffer(slab,dtype='<f2',count=128,offset=131072).copy()
 ...
 oc=num.fp8_rn_satfinite(out);physical=np.empty(8192,dtype=np.uint8);physical[C_MAP]=oc
 gp=np.empty(8192,dtype=np.uint8);gp[C_MAP]=gcode
