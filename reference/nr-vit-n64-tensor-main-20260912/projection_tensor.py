...
M,K,N=64,1024,1024
SOURCE=V/'nr-vit-projection-main-20260912/original.ptx'
SOURCE_SHA='07fb4c01932ede72fdfe0f82d47063c0be5aa1dbaabf18a13b5f568d3b959e50'
...
def forward(p0,p8,slab,hooks=None):
 for b,size in ((p0,65536),(p8,65536),(slab,1050624)):
 ...
 coefficient=np.frombuffer(slab,dtype='<f2',offset=1048576).copy()
 ...
 for z in range(4):
  ...
  for k0 in range(z*256,(z+1)*256,32):
...
def main():
 p0=(V/'nr-vit-attention-main-20260912/output_fp8_physical.bin').read_bytes();p8=(V/'nr-vit-contract-main-20260912/output_fp8_physical.bin').read_bytes()
 ck=HERE.parent/'weights/dlssnr-310.8-weights.safetensors'
 with ck.open('rb') as f:f.seek(47154884);slab=f.read(1050624)
 assert hashlib.sha256(slab).hexdigest()=='99fdd6de02b14af6c61ff13a103405c26fe1f4caa555dbc9b5236b6d660318b4'
