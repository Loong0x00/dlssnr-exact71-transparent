...
class RsqrtHalfDomain:
 ...
 @classmethod
 def load(cls):return cls((HERE/'rsqrt-f32.bin').read_bytes())
 def evaluate(self,x):
...
def forward(raw,slab,*,rsqrt,hooks=None):
 ...
 if type(slab)is not bytes or len(slab)!=21696 or not isinstance(rsqrt,RsqrtHalfDomain):raise ValueError('own slab and measured policy required')
 q=tree(raw[:,:,0,:],rsqrt);k=tree(raw[:,:,1,:],rsqrt)
 sf=np.frombuffer(slab,dtype='<f4',count=1,offset=20576).copy();s=num._rn_half(sf.astype(np.float64),'Q scale f32 to half')
