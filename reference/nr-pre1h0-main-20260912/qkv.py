"""Own pre0 raw pre-normalization64x96 projection; bank identities require consumers.
...
A_MAP=axes.a_offset(np.arange(64)[:,None],np.arange(32)[None,:],32)
B_MAP=9312+axes.b_offset(np.arange(32)[:,None],np.arange(96)[None,:],96)
def forward_raw(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=2048 or type(slab)is not bytes or len(slab)!=21696:raise ValueError('own64x32/slab contract')
 a=num.decode_e4m3(features,A_MAP);b=num.decode_e4m3(slab,B_MAP);c=np.zeros((64,96),dtype=np.float16);before={};after={}
 ...
 banks=c.reshape(64,1,3,32);num._call_hook(hooks,'raw_projected_half',c);num._call_hook(hooks,'warp_bank_view',banks)
