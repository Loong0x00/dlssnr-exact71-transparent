...
def tree(x,rsqrt):
 ...
 local=rn(pa.astype(np.float64)+pb.astype(np.float64),'norm local').reshape(64,8,4,2)
...
def forward(raw,slab,*,rsqrt,hooks=None):
 if not isinstance(raw,np.ndarray) or raw.dtype!=np.float16 or raw.shape!=(64,8,3,32) or not np.all(np.isfinite(raw)):raise ValueError('finite raw64x8x3x32 half input required')
 if type(slab)is not bytes or len(slab)!=820288 or not isinstance(rsqrt,RsqrtHalfDomain):raise ValueError('own slab and measured policy required')
 q=tree(raw[:,:,0,:],rsqrt);k=tree(raw[:,:,1,:],rsqrt)
 sf=np.frombuffer(slab,dtype='<f4',count=8,offset=623136).copy();s=num._rn_half(sf.astype(np.float64),'Q scale f32 to half')
