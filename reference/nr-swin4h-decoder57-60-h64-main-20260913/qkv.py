...
B_MAP=98592+axes.b_offset(np.arange(128)[:,None],np.arange(384)[None,:],384)
def forward_raw(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=8192 or type(slab)is not bytes or len(slab)!=197184:raise ValueError('own64x256/slab contract')
