...
def forward(Q,K,Vv,skip,slab,*,reciprocal,hooks=None):
 ...
 if not isinstance(skip,np.ndarray) or skip.dtype!=np.float16 or skip.shape!=(64,32) or type(slab)is not bytes or len(slab)!=22720 or not isinstance(reciprocal,ReciprocalHalfDomain):raise ValueError('own skip/slab/policy required')
