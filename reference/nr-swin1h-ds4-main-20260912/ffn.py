...
Consumes/produces canonical-C packed64x32 FP8. Separate original-operand acceptance.
"""
...
def forward(x,slab,hooks=None):
 if not isinstance(x,np.ndarray) or x.dtype!=np.float16 or x.shape!=(64,32) or not np.all(np.isfinite(x)):raise ValueError('finite decoded P0 canonical64x32 half')
 if type(slab)is not bytes or len(slab)!=22720:raise ValueError('own1h22720byte slab')
 code=num.fp8_rn_satfinite(x);values=num.decode_e4m3(code.tobytes(),np.arange(2048).reshape(64,32));a=values[:,pi(np.arange(32))]
 we=num.decode_e4m3(slab,EXP_MAP);wc=num.decode_e4m3(slab,CONTRACT_MAP);gamma=np.frombuffer(slab,dtype='<f2',count=32,offset=8208).copy()
 ...
 seed=num._rn_half(x.astype(np.float64)*gamma.astype(np.float64),'decodedP0 earlyFFNseed');out=seed.copy();before={};after={}
 for k in (0,32,64,96):
