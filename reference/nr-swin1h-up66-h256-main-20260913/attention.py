...
A_MAP=axes.a_offset(m,c,32);C_MAP=axes.c_offset(m,c,32);B_MAP=21680+axes.b_offset(c.T,c,32)
h=np.arange(1)[:,None,None];q=np.arange(64)[None,:,None];k=np.arange(64)[None,None,:]
BIAS_MAP=13472+8192*h+512*(4*(q//16)+k//16)+16*(4*(q%8)+(k%8)//2)+8*((k%16)//8)+4*((q%16)//8)+2*(k%2)
...
def forward(Q,K,Vv,skip,slab,*,reciprocal,hooks=None):
 ...
 if not isinstance(skip,np.ndarray) or skip.dtype!=np.float16 or skip.shape!=(64,32) or type(slab)is not bytes or len(slab)!=22784 or not isinstance(reciprocal,ReciprocalHalfDomain):raise ValueError('own skip/slab/policy required')
 ...
 ma=num.decode_e4m3(pv_phys.tobytes(),A_MAP);weight=num.decode_e4m3(slab,B_MAP);residual=skip.astype(np.float64);gamma=np.frombuffer(slab,dtype='<f2',count=32,offset=22704).copy()
