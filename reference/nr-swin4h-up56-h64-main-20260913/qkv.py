...
A_MAP=axes.a_offset(np.arange(64)[:,None],np.arange(128)[None,:],128)
B_MAP=131584+axes.b_offset(np.arange(128)[:,None],np.arange(384)[None,:],384)
def forward_raw(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=8192 or type(slab)is not bytes or len(slab)!=230176:raise ValueError('own64x256/slab contract')
 a=num.decode_e4m3(features,A_MAP);b=num.decode_e4m3(slab,B_MAP);c=np.zeros((64,384),dtype=np.float16);before={};after={}
 ...
 banks=c.reshape(64,4,3,32);num._call_hook(hooks,'raw_projected_half',c);num._call_hook(hooks,'warp_bank_view',banks)
