...
m=np.arange(64)[:,None];c=np.arange(128)[None,:]
A_MAP=axes.a_offset(m,c,128);C_MAP=axes.c_offset(m,c,128);B_MAP=213520+axes.b_offset(c.T,c,128)
h=np.arange(4)[:,None,None];q=np.arange(64)[None,:,None];k=np.arange(64)[None,None,:]
BIAS_MAP=180736+8192*h+512*(4*(q//16)+k//16)+16*(4*(q%8)+(k%8)//2)+8*((k%16)//8)+4*((q%16)//8)+2*(k%2)
...
def forward(Q,K,Vv,skip,slab,*,reciprocal,hooks=None):
 for a in (Q,K,Vv):
  if not isinstance(a,np.ndarray) or a.dtype!=np.uint8 or a.shape!=(64,4,32):raise ValueError('normalized logical64x8x32 FP8 arrays required')
 if type(skip)is not bytes or len(skip)!=8192 or type(slab)is not bytes or len(slab)!=230176 or not isinstance(reciprocal,ReciprocalHalfDomain):raise ValueError('own skip/slab/policy required')
 decode=lambda x:num.decode_e4m3(x.tobytes(),np.arange(8192).reshape(64,4,32)).transpose(1,0,2)
 ...
 idx=pi(np.arange(32));score=np.stack([num._rn_half(a[h][:,idx]@b[h][:,idx].T+bias[h].astype(np.float64),'seeded QK K32') for h in range(4)])
 ...
 pairs=clamp.view(np.uint16).reshape(4,64,32,2).astype(np.uint32);packed=pairs[...,0]|(pairs[...,1]<<16)
 shifted=(packed.astype(np.uint64)<<5)&0xffffffff;word=((shifted+2146992128)&0xffffffff).astype(np.uint32)
 mapped_bits=np.stack([word&65535,word>>16],axis=-1).astype(np.uint16).reshape(4,64,64);mapped=mapped_bits.view(np.float16)
 ...
 local01=add(pair[0],pair[1],'mapped local01');local012=add(local01,pair[2],'mapped local012');local=add(local012,pair[3],'mapped local0123').reshape(4,64,4,2)
 ...
 pv_input=num.decode_e4m3(pcodes.tobytes(),np.arange(16384).reshape(4,64,64));pv=np.zeros((4,64,32),dtype=np.float16);pv_before={};pv_after={}
 for k0 in (0,32):
  pv_before[k0]=pv.copy();ii=pi(np.arange(k0,k0+32))
  pv=np.stack([num._rn_half(pv_input[h][:,ii]@v[h][ii]+pv[h].astype(np.float64),'PV perK32') for h in range(4)]);pv_after[k0]=pv.copy()
 flat=pv.transpose(1,0,2).reshape(64,128);pv_codes=num.fp8_rn_satfinite(flat);pv_phys=np.empty(8192,dtype=np.uint8);pv_phys[C_MAP]=pv_codes
 ma=num.decode_e4m3(pv_phys.tobytes(),A_MAP);weight=num.decode_e4m3(slab,B_MAP);residual=num.decode_e4m3(skip,C_MAP);gamma=np.frombuffer(slab,dtype='<f2',count=128,offset=229904).copy()
 ...
 code=num.fp8_rn_satfinite(out);physical=np.empty(8192,dtype=np.uint8);physical[C_MAP]=code
