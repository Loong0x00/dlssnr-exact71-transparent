"""Own1h OUTVIEW NN: full-window core + guarded physical-A output.
...
PI=engine.attention.pi(np.arange(32));yy,xx=np.meshgrid(np.arange(8),np.arange(8),indexing='ij');mm=16*((yy//4)*2+xx//4)+4*(yy%4)+xx%4
C_MAP=engine.ffn.axes.c_offset(mm[:,:,None],np.arange(32)[None,None,:],32)
def extents(h,w,output_extent):
 ...
 if not(1<=oh<=256 and 1<=ow<=256):raise ValueError('bounded effective output extents')
 return oh,ow

def forward(features,slab,*,h=64,w=64,x=0,y=-4,output_extent=(0,0),rsqrt,reciprocal,keep_stages=False):
 ...
 out=np.empty((2,oh,ow,16),dtype=np.uint8);seen=np.zeros((oh,ow),dtype=np.bool_);stages={}
 for cy in range(gy):
  for cx in range(gx):
   ...
   for ly in range(8):
    for lx in range(8):
     ...
     if 0<=gy0<oh and 0<=gx0<ow:
      ...
      seen[gy0,gx0]=True;out[:,gy0,gx0,:]=logical[ly,lx,PI].reshape(2,16)
   if keep_stages:z['ingress']=inp;stages[(cx,cy)]=z
 if not np.all(seen):raise ValueError('unproduced output')
 return dict(output=out.tobytes(),output_A_basis=out.transpose(1,2,0,3).reshape(oh,ow,32).copy(),extent=(oh,ow),stages=stages)
