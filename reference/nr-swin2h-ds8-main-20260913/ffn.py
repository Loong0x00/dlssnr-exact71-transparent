...
def forward(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=4096 or type(slab)is not bytes or len(slab)!=69936:raise ValueError('own64x256/slab byte contract')
 ...
 gamma=np.frombuffer(slab,dtype='<f2',count=64,offset=28688).copy()
