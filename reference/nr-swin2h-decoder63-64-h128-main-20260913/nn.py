"""Ordinary2h source-equation NN: full64-query windows, original sourcezero ingress.
...
def geometry(h,w,x,y):
 if any(type(i)is not int for i in (h,w,x,y)) or not(8<=h<=128 and 8<=w<=128 and h%8==w%8==0 and x in (-4,0) and y in (-4,0)):raise ValueError('admitted positive multiples8 and XY0/-4')
...
def tile_sources(h,w,x,y,cx,cy):
 ...
 off=np.arange(4096);tile=off//1024;ty=(y+8*cy)//4+tile//2;tx=(x+8*cx)//4+tile%2;valid=(ty>=0)&(ty<h//4)&(tx>=0)&(tx<w//4)
 return np.where(valid,1024*(ty*(w//4)+tx)+off%1024,-1)

def ingress(features,*,h,w,x,y,cta):
 if type(features)is not bytes or len(features)!=h*w*64:raise ValueError('canonical-C64 packed feature span')
 mp=tile_sources(h,w,x,y,*cta);out=np.zeros(4096,dtype=np.uint8);valid=mp>=0;out[valid]=np.frombuffer(features,dtype=np.uint8)[mp[valid]];return out
...
def forward(features,slab,*,h=32,w=32,x,y,rsqrt,reciprocal,keep_stages=False):
 gx,gy=geometry(h,w,x,y);out=np.empty(h*w*64,dtype=np.uint8);seen=np.zeros(h*w*64,dtype=np.bool_);stages={}
 for cy in range(gy):
  for cx in range(gx):
