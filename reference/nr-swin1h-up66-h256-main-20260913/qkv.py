...
B_MAP=10400+axes.b_offset(np.arange(32)[:,None],np.arange(96)[None,:],96)
def forward_raw(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=2048 or type(slab)is not bytes or len(slab)!=22784:raise ValueError('own64x256/slab contract')
