...
A_MAP=axes.a_offset(m,c,256);C_MAP=axes.c_offset(m,c,256);B_MAP=623168+axes.b_offset(c.T,c,256)
h=np.arange(8)[:,None,None];q=np.arange(64)[None,:,None];k=np.arange(64)[None,None,:]
BIAS_MAP=557600+8192*h+512*(4*(q//16)+k//16)+16*(4*(q%8)+(k%8)//2)+8*((k%16)//8)+4*((q%16)//8)+2*(k%2)
...
def forward(Q,K,Vv,skip,slab,*,reciprocal,hooks=None):
 ...
 if type(skip)is not bytes or len(skip)!=16384 or type(slab)is not bytes or len(slab)!=820288 or not isinstance(reciprocal,ReciprocalHalfDomain):raise ValueError('own skip/slab/policy required')
 ...
 ma=num.decode_e4m3(pv_phys.tobytes(),A_MAP);weight=num.decode_e4m3(slab,B_MAP);residual=num.decode_e4m3(skip,C_MAP);gamma=np.frombuffer(slab,dtype='<f2',count=256,offset=688704).copy()
