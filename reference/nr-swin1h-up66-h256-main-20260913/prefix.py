"""Own2h source-derived local prefix64→32; actual operand proof is separate."""
...
m=np.arange(16)[:,None];k=np.arange(64)[None,:];n=np.arange(32)[None,:]
A_MAP=16*((k//16*4+m//4)*4+m%4)+k%16
B_MAP=8192+axes.b_offset(k.T,n,32)
y,x=np.meshgrid(np.arange(8),np.arange(8),indexing='ij');tile=16*((y//4)*2+x//4)+4*(y%4)+x%4
C_MAP=axes.c_offset(tile[:,:,None],np.arange(32)[None,None,:],32)
def forward(source,skip,slab,hooks=None):
 for b,nbytes in ((source,1024),(skip,2048),(slab,22784)):
  if type(b)is not bytes or len(b)!=nbytes:raise ValueError('4h local prefix spans')
 a=num.decode_e4m3(source,A_MAP);b=num.decode_e4m3(slab,B_MAP);s=num.decode_e4m3(skip,C_MAP);gamma=np.frombuffer(slab,dtype='<f2',count=32,offset=10336).copy()
 if not np.all(np.isfinite(gamma)):raise num.UnsupportedNonFinite('prefix gamma')
 carry=np.zeros((16,32),dtype=np.float16);before={};after={}
 ...
 low=carry.reshape(4,4,32);near=low.repeat(2,0).repeat(2,1);scaled=num._rn_half(s*gamma.astype(np.float64),'4h late skip');pre=num._rn_half(near.astype(np.float64)+scaled.astype(np.float64),'4h late add');codes=num.fp8_rn_satfinite(pre);physical=np.empty(2048,dtype=np.uint8);physical[C_MAP]=codes
