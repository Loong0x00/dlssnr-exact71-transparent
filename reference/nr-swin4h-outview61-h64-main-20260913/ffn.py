...
def forward(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=8192 or type(slab)is not bytes or len(slab)!=197184:raise ValueError('own64x256/slab byte contract')
 ...
 gamma=np.frombuffer(slab,dtype='<f2',count=128,offset=98320).copy()
