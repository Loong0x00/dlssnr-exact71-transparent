...
EXP_MAP=32768*(h//128)+axes.b_offset(k.T,h%128,128)
GROUP_MAP=262144+4096*np.arange(8)[:,None,None]+axes.b_offset(np.arange(128)[None,:,None],np.arange(32)[None,None,:],32)
MIX_MAP=294912+axes.b_offset(k.T,k,C)
...
def forward(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=16384 or type(slab)is not bytes or len(slab)!=820784:raise ValueError('own64x256/slab byte contract')
 ...
 gamma=np.frombuffer(slab,dtype='<f2',count=256,offset=491520).copy()
 ...
 oc=num.fp8_rn_satfinite(out);physical=np.empty(16384,dtype=np.uint8);physical[C_MAP]=oc
 gp=np.empty(16384,dtype=np.uint8);gp[C_MAP]=gcode
