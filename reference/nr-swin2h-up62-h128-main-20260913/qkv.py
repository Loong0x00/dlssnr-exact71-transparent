...
B_MAP=37120+axes.b_offset(np.arange(64)[:,None],np.arange(192)[None,:],192)
def forward_raw(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=4096 or type(slab)is not bytes or len(slab)!=70048:raise ValueError('own64x256/slab contract')
