...
B_MAP=360992+axes.b_offset(np.arange(256)[:,None],np.arange(768)[None,:],768)
def forward_raw(features,slab,hooks=None):
 if type(features)is not bytes or len(features)!=16384 or type(slab)is not bytes or len(slab)!=820288:raise ValueError('own64x256/slab contract')
