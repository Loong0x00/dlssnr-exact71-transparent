...
def forward(raw,slab,*,rsqrt,hooks=None):
 ...
 if type(slab)is not bytes or len(slab)!=22784 or not isinstance(rsqrt,RsqrtHalfDomain):raise ValueError('own slab and measured policy required')
 q=tree(raw[:,:,0,:],rsqrt);k=tree(raw[:,:,1,:],rsqrt)
 sf=np.frombuffer(slab,dtype='<f4',count=1,offset=21664).copy();s=num._rn_half(sf.astype(np.float64),'Q scale f32 to half')
