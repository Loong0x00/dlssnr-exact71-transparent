...
def forward(Q,K,Vv,skip,slab,*,reciprocal,hooks=None):
 ...
 if type(skip)is not bytes or len(skip)!=8192 or type(slab)is not bytes or len(slab)!=229936 or not isinstance(reciprocal,ReciprocalHalfDomain):raise ValueError('own skip/slab/policy required')
