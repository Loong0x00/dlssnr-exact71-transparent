"""Pre0 FFN: unquantized embedding seed, FP8 expansion, cubic and four K32 carries.
Logical64x32 half input;128 hidden neurons; canonicalC packedFP8 output.
"""
import numpy as np
...
def forward(x,slab,hooks=None):
 ...
 if type(slab)is not bytes or len(slab)!=21696:raise ValueError('own1h21696byte slab')
 code=num.fp8_rn_satfinite(x);values=num.decode_e4m3(code.tobytes(),np.arange(2048).reshape(64,32));a=values[:,pi(np.arange(32))]
 we=num.decode_e4m3(slab,EXP_MAP);wc=num.decode_e4m3(slab,CONTRACT_MAP);gamma=np.frombuffer(slab,dtype='<f2',count=32,offset=9232).copy()
